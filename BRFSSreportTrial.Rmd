...
Next we load all of the R libraries we'll be using in this analysis.  No need to show the results in the report so we set `echo=F`.
...
Our map was saved in:
```{r MapPath}
path <- getwd()
paste(path,"/healthy_sleepers_by_state_2014.png")  

```

Showing the map as a figure:  **TODO:  Need to know how to display saved figures**
```{r DisplayMapFigure, eval=F}
![](paste(path,"/healthy_sleepers_by_state_2014.png"))
```
